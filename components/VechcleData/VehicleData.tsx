...
import ConfirmationPopUp from "../common/confirmation/ConfirmationPopUp";
...
const VechicleDataC: React.FC<VehicleDataProps> = ({ isOpen, reRender }) => {
  ...
  const onDelete = async () => {
    await fetch(`api/vehicles/${(selectedVehicle as Vehicle)?.id}`, {
      method: "DELETE",
    });
  ...
  return (
    <Card sx={{ maxWidth: 400, minWidth: 350 }}>
      ...
      <CardActions>
        <ConfirmationPopUp
          mainVal="Delete"
          secondaryVal="Are you sure you want to delete this vehicle?"
          headerVal="Delete Vehicle"
